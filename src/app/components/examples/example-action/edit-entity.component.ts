...
import { AttributeScope, EntitySearchDirection, EntityType, PageComponent, RelationTypeGroup } from "@shared/public-api";
...
export class EditEntityComponent extends PageComponent implements OnInit, OnDestroy {
  ...
  private saveEntityObservable(): Observable<Asset | Device> {
    const formValues = this.editEntityFormGroup.value;
    const entity = {
  ...
  private saveRelations(entityId: EntityId): Observable<EntityRelation[]> {
    const relations = this.editEntityFormGroup.get("relations").value;
