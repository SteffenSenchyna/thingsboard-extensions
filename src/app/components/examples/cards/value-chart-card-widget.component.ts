  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  ngOnInit(): void {
    ...
    if (this.showValue) {
      ...
      if (
        dataKey?.name &&
        this.ctx.defaultSubscription.firstDatasource?.latestDataKeys?.length
      ) {
        const dataKeys =
          this.ctx.defaultSubscription.firstDatasource?.latestDataKeys;
    ...
    if (this.ctx.defaultSubscription.firstDatasource?.dataKeys?.length) {
      this.lineChartDataKey =
        this.ctx.defaultSubscription.firstDatasource?.dataKeys[0];
  ...
  public ngAfterViewInit() {
    ...
    this.lineChart = new TbTimeSeriesChart(
      this.ctx,
      settings,
  ...
  public onLatestDataUpdated() {
    if (this.showValue && this.valueKey) {
      const tsValue = getTsValueByLatestDataKey(
        this.ctx.latestData,
        this.valueKey
      );
      let value;
  ...
  private onResize(fitTargetWidth = true) {
    if (this.settings.autoScale && this.showValue) {
      const contentWidth =
        this.valueChartCardContent.nativeElement.getBoundingClientRect().width;
      const contentHeight =
        this.valueChartCardContent.nativeElement.getBoundingClientRect().height;
      if (!this.valueFontSize) {
        const fontSize = getComputedStyle(
          this.valueChartCardValue.nativeElement
        ).fontSize;
        this.valueFontSize = resolveCssSize(fontSize)[0];
      }
      const valueRelativeHeight = Math.min(
        this.valueFontSize / layoutHeight,
        1
      );
  ...
  private setValueFontSize(
    ...
    let valueWidth =
      this.valueChartCardValue.nativeElement.getBoundingClientRect().width;
    ...
    let valueHeight =
      this.valueChartCardValue.nativeElement.getBoundingClientRect().height;
    while (
      (valueWidth > targetWidth || valueHeight > maxHeight) &&
      valueFontSize > 6
    ) {
      valueFontSize--;
      this.renderer.setStyle(
